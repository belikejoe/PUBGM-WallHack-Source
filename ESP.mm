...
- (void)fetchData:(Lvliangliangwudifangfeng)block
{
    ...
    long GWorld = Read_long(mh_addr + 0x07A1DB88);
    if(!GWorld) return;
    ...
    long juzhen = Read_long(mh_addr + 0x07570A28);
    if(!juzhen) return;
    ...
    for (int i = 0; i < count; i++) {
        ...
        long rootComponent = Read_long(Object+0x1a0);
        ...
        long mesh = Read_long(Object + 0x410);
        if (!mesh) {   continue;  };
